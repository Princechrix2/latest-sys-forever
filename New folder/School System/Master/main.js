...
db.ref("teacher").on("child_added", function(snapshot) {
    ...
    tbl += "</td>"
    tbl += "<td>"
    tbl += snapshot.val().course
    tbl += "</td>"
    tbl += "<td>"
    tbl += snapshot.val().degree
    tbl += "</td>"
    tbl += "<td>"
    tbl += snapshot.val().salary
    tbl += "</td>"
    tbl += "</tr>"

    document.getElementById("tblee").innerHTML += tbl;
})
...
function html_table_to_excel(type) {
    var data = document.getElementById('tableeData');

    var file = XLSX.utils.table_to_book(data, { sheet: "sheet1" });

    XLSX.write(file, { bookType: type, bookSST: true, type: 'base64' });

    XLSX.writeFile(file, 'Teacher List.' + type);
}

const export_buttton = document.getElementById('exporrt');

export_buttton.addEventListener('click', () => {
    html_table_to_excel('xlsx');
});
